...
**Last updated:** 2026-07-27 — pywebview Windows-chrome fixes tested live, then a pywebview→Electron migration decision and Phase 1 spike, both verified live on Kevin's Windows machine this session
**Status:** The 2026-07-26 UI rework (review step, Pill mode, frameless window) was tested live for the first time this session on Windows and found broken — no frameless effect, grey border, tiny/unusable pill. Root-caused and fixed (commit `f858270`): pywebview's `transparent=True` never actually composites on Windows (WinForms Form background falls back to system grey), replaced with DWM native rounded corners + a GDI capsule region for Pill mode; real window drag added via a `move_window_by` bridge; a CSS margin bug that left a 44px pill only ~12px of usable height fixed; pill resized/restyled. Verified live via window-scoped screenshots (not full-desktop) and a simulated drag confirmed via `GetWindowRect`.

Kevin then found a *second* grey border still present plus an unstyled white Settings scrollbar, and asked to stop and research rather than keep patching — that research found pywebview has **documented, unresolved GitHub issues** (r0x0r/pywebview #1413, #1611) matching our exact symptoms, and confirmed Wispr Flow itself is built on Electron. Kevin chose to migrate the UI shell to Electron. A phased plan was written (`C:\Users\admin\.claude\plans\lexical-wondering-spring.md` on the machine that authored it — not committed, per this repo's convention that mockups/plans aren't repo artifacts) and Phase 1 (a no-Python "shape spike": does Electron actually render a clean borderless capsule) was implemented and verified live this session — see below. **Not yet done:** Phase 2 (strip pywebview from `main.py`, add stdio-JSON IPC to a spawned Python backend) — gated behind an explicit effort-level go-ahead from Kevin per this file's own governance, not started.
