...
**Last updated:** 2026-09-04 — **Oversized-orb Mini Float bug (flagged 2026-08-10/11, never confirmed) fixed: `maximizable: false` added to `createWindow()`'s `BrowserWindow` constructor (`electron/main.js`), Kevin-confirmed live.** Kevin reported the regression again — screenshots showed the pill as a large dark circle overlapping other windows, including Hope's assistant panel in a browser. This is the exact bug from the 10-11 Aug entries below: the `ec36605` native drag-region (`-webkit-app-region: drag`) permits double-click/Snap-maximize independent of `resizable: false`, and `.pill-bar`'s `border-radius: 999px` renders any oversized near-square window as a blob. On 11 Aug this fix was identified but deliberately withheld because four synthetic repro attempts (double-click x2, Win+Up snap, drag-to-edge) failed to trigger the bug, so it couldn't be confirmed live at the time. Applied now as a defensive fix regardless of the exact trigger (commit `f4162ed`, branch `fix-mini-float-maximizable`, merged to `main` `a274705`). **Kevin tested drag/double-click/Snap live after merge — works fine, no recurrence.** See the "FOLLOW-UP 2026-08-11" entry further down for the full investigation trail this closes out.
...
**RESOLVED 2026-09-04.** Bug recurred (Kevin screenshots, oversized pill overlapping Hope's browser assistant panel). Applied the withheld `maximizable: false` fix to `createWindow()`'s `BrowserWindow` constructor (`electron/main.js`) — commit `f4162ed` on `fix-mini-float-maximizable`, merged to `main` at `a274705`. **Kevin tested live (drag, double-click, Snap) — confirmed working, no recurrence.** The "not yet done" items above (CSS transition check, screen recording) are now moot unless this somehow recurs again despite the fix.
