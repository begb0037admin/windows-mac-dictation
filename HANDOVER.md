...
**Last updated:** 2026-09-04 (correction, same day) — **The oversized-circle bug RECURRED after the `maximizable: false` fix — that fix was NOT the root cause. Kevin sent a screen recording; frame-by-frame it shows the real trigger: clicking the Full window's Mini-Bar-Mode ("minimize") button.** Sequence: Full window renders correctly → click Mini Bar Mode (`btnPill`) → within ~1 frame the window balloons into the oversized dark circle, instead of collapsing to the small pill. Root cause: `ui/app.js`'s `enablePillMode()` added the `pill-mode`/`mode-pill` CSS classes **synchronously**, then sent the native resize over IPC (`resizeWindow`, `ipcRenderer.send` — fire-and-forget) **asynchronously**. `body.pill-mode .app` (`ui/styles.css`) derives its height from `100vh` and switches `border-radius` to `999px` — so for however many frames the real native window lagged behind the CSS flip, `.app` was still the old (larger, near-square) size with a 999px radius applied: an oversized circle. `maximizable: false` (`f4162ed`/`a274705`, kept — harmless, just not the cause here) targeted a native OS maximize/Snap path that was never the actual trigger. **Fix (not yet merged):** `electron/preload.js`'s `resizeWindow` now uses `ipcRenderer.invoke`; `electron/main.js`'s `resize-window` handler is `ipcMain.handle`; `enablePillMode()`/`disablePillMode()` now `await` the resize before touching any CSS class, so `100vh` already reflects the target size when the border-radius/height rules switch. Branch `fix-pill-collapse-resize-race` @ `b8fa2bc`, pushed, **not merged** — `node --check` clean on all three files, not live-verified. Kevin needs to reproduce the exact repro (expand to Full, click Mini Bar Mode) and confirm a clean collapse before merge. See the "FOLLOW-UP 2026-08-11" entry further down (its unchecked action item — check for a CSS transition/race on the pill↔full toggle — is exactly what this was).
...
**PARTIALLY RESOLVED 2026-09-04, then CORRECTED same day.** First recurrence (Kevin screenshots, oversized pill overlapping Hope's browser assistant panel) was fixed with the withheld `maximizable: false` change (`electron/main.js`'s `createWindow()`) — commit `f4162ed` on `fix-mini-float-maximizable`, merged to `main` at `a274705`, Kevin-confirmed live for drag/double-click/Snap. **But the bug recurred again the same day via a different trigger Kevin caught on a screen recording: clicking the Full window's Mini Bar Mode (minimize) button.** This is exactly the unchecked item above — a CSS-vs-native-resize race on the pill↔full toggle, not a native maximize/Snap path. Root cause and fix (unmerged, pending Kevin's test) are in the new top-of-file entry dated 2026-09-04 (correction). `maximizable: false` stays (harmless) but was never the real cause of the ballooning-circle failure mode.
