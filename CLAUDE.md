...
- **Status:** **Windows is done and locked in (2026-07-29)** — packaged installer (Electron shell + PyInstaller-frozen backend, NSIS) built, installed, and confirmed fully working end-to-end by Kevin: hotkey (keyboard or mouse button), always-on-top, system tray minimize/close/exit, run-on-login. Do not re-open or re-litigate the Windows build pipeline without a new concrete bug report. **Mac is next and is completely unverified** — Decision 7's groundwork exists (`build/push2talk-backend.mac.spec`, `build/build-backend.sh`, `build/build-app.sh`, `build/lock/mac-arm64.in`) but has never been run; real Apple Silicon hardware is required and is not available in this Windows-only environment. See `HANDOVER.md` for the full Windows session record and outstanding Mac plan. File transcription removed — it belongs to `meeting-transcriber`.
