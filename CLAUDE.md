...
- **Status:** **Windows and Mac both installed and Kevin-confirmed working as of 2026-09-05, both on the "PTT" name.** Windows: `PTT 0.1.3` installer, Kevin-confirmed PASS 2026-08-24 (see HANDOVER.md). Mac: `PTT-0.1.3-arm64.dmg` built end-to-end from `main` (`11b70a9`) on Kevin's own Apple Silicon machine, installed to `/Applications/PTT.app`, Kevin confirmed "it works" 2026-09-05 — this closes out the previous "no current Mac installer" gap and the Mac cold-start "stuck on Transcribing…" report (root cause: first-run ~1.5GB Whisper weight download with no UI feedback, not a regression; fixed with an additive "preparing model" status + watchdog caption, PR #2). Old `/Applications/Push 2 Talk.app` (pre-rename, 2026-08-05) kept on Kevin's machine as a fallback at his request — not to be removed without asking. See `HANDOVER.md`. File transcription removed — it belongs to `meeting-transcriber`.
