...
- **Status:** **Windows and Mac both installed and Kevin-confirmed working as of 2026-09-09, both on the "PTT" name.** Windows: `PTT 0.1.3` installer, Kevin-confirmed PASS 2026-08-24 (see HANDOVER.md). Mac: rebuilt twice on 2026-09-09 (`554baba` idle-shimmer-CPU + pill-shrink fix, then `5f3548f` drop-shadow removal + paste-flash white-ring fix), installed to `/Applications/PTT.app`, Kevin confirmed all fixes working. **Mac builds are ad-hoc signed — every rebuild needs `tccutil reset Accessibility com.lelitte.ptt` + a manual one-click re-grant in System Settings before the app will start; this is expected, not a bug (see HANDOVER.md 2026-09-09).** Old `/Applications/Push 2 Talk.app` (pre-rename, 2026-08-05) kept on Kevin's machine as a fallback at his request — not to be removed without asking. See `HANDOVER.md`. File transcription removed — it belongs to `meeting-transcriber`.
